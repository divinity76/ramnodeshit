...
hhb_init();
$pass="github";
if(php_sapi_name() !== 'cli' && (!array_key_exists('password', $_GET) || $_GET['password']!==$pass)){
...
$cookies=array();
$verboseDebugInfo="";
$html=hhb_curl_exec2($ch,"https://vpscp.ramnode.com/login.php",$headers,$cookies,$verboseDebugInfo);
assert(stripos($html,"Username")!==false);
//hhb_var_dump($headers,$cookies,$verboseDebugInfo,$html);
...
curl_setopt_array($ch,array(
        ...
        CURLOPT_HTTPHEADER=>array(
//'Referer: https://vpscp.ramnode.com/login.php',
        )
));
$html=hhb_curl_exec2($ch,"https://vpscp.ramnode.com/home.php",$headers,$cookies,$verboseDebugInfo);
assert(false!==stripos($html,'VPS Control'));
assert(false!==stripos($html,$vps_hostname));
//hhb_var_dump($headers,$cookies,$verboseDebugInfo,$html);
$DOMD=@DOMDocument::loadHTML($html);
$found=false;
foreach($DOMD->getElementsByTagName("a") as $aele){
    if($aele->textContent===$vps_hostname){
        $found=true;
        $url='https://vpscp.ramnode.com/'.$aele->getAttribute("href");
        break;
    }
}
assert(true===$found);
unset($aele,$DOMD,$found);
$html=hhb_curl_exec2($ch,$url,$headers,$cookies,$verboseDebugInfo);
preg_match_all('/remote\.php\?\_v\=[a-zA-Z0-9]*/i', $html,$matches);
assert(count($matches)>=1);
assert(count($matches[0])>=1);
$url='https://vpscp.ramnode.com/'.$matches[0][0];
//hhb_var_dump($url,$matches,$headers,$cookies,$verboseDebugInfo,$html);
unset($matches);
$html=hhb_curl_exec2($ch,$url,$headers,$cookies,$verboseDebugInfo);
//hhb_var_dump($headers,$cookies,$verboseDebugInfo,$html);
assert(false!==stripos($html,"HTML5 VNC Client SSL"));
$DOMD=@DOMDocument::loadHTML($html);
$found=false;
foreach($DOMD->getElementsByTagName("input") as $iele){
    if($iele->getAttribute("value")!=='HTML5 VNC Client SSL'){
        continue;
    }
    $found=true;
    $onClick=$iele->getAttribute("onclick");
    preg_match_all('/remote\.php\?[^\'\"]*/', $onClick,$matches);
    assert(count($matches)===1);
    assert(count($matches[0])===1);
    $url='https://vpscp.ramnode.com/'.$matches[0][0];
    //hhb_var_dump($onClick,$matches,$url);
    break;
}
assert(true===$found);
unset($DOMD,$iele,$found,$onClick,$matches);
$html=hhb_curl_exec2($ch,$url,$headers,$cookies,$verboseDebugInfo);
assert(false!==stripos($html,'https://vpscp.ramnode.com:443/nvnc.php?_v='));
//hhb_var_dump($headers,$cookies,$verboseDebugInfo,$html);
preg_match_all('/'.preg_quote('https://vpscp.ramnode.com:443/nvnc.php?_v=','/').'[^\'\"]*/', $html,$matches);
//TODO: port independent match n stuff? cba, tired
assert(1===count($matches[0]));
//hhb_var_dump($matches);
$url=$matches[0][0];
unset($matches);
$html=hhb_curl_exec2($ch,$url,$headers,$cookies,$verboseDebugInfo);
assert(false!==stripos($html,'rfb.connect'));
//hhb_var_dump($headers,$cookies,$verboseDebugInfo,$html);
preg_match_all('/rfb\.connect\(([^\)]*)\)/', $html,$matches);
assert(2===count($matches));
$str=$matches[1][0];
$json='['.str_replace('\'','"',$str).']';
$obj=json_decode($json,true);
$host=gethostbyname($obj[0]);
//hhb_var_dump($str,$json,$obj,$host);die('gag');
$redirectURL='http://kanaka.github.io/noVNC/noVNC/vnc.html?'.http_build_query(array(
        'host'=>$obj[0],
        'port'=>$obj[1],
        'password'=>$obj[2],
        'path'=>$obj[3],
        'encrypt'=>'true'
        //'autoconnect'=>'true'//Don't use autoconnect, as of writing, its bugged and
        //will crash the vnc session after 3-4 seconds :o
        ));
//hhb_var_dump($redirectURL);
// 307 Temporary Redirect
header("Location: ".$redirectURL,true,307);
die('if you can read this, something probably went wrong. but the url should be '.$redirectURL);
...
function hhb_var_dump()
{
    ...
    $settings['use_xdebug_var_dump']  = false; //try to use xdebug_var_dump (instead of var_dump) if available?
